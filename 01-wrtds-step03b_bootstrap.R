...
dir.create(path = file.path(path, "WRTDS Bootstrap Diagnostic Coastal Sites"), showWarnings = F)
dir.create(path = file.path(path, "WRTDS Bootstrap Outputs 2026"), showWarnings = F)
dir.create(path = file.path(path, "WRTDS Bootstrap Outputs Coastal Sites"), showWarnings=F)
...
coastal_sites <- read.csv(file.path(path, "WRTDS Inputs", "coastal_sites_list.csv"))
...
# Identify all rivers that make it through the regular workflow
all_good_rivers <- data.frame("file" = dir(path = file.path(path, "WRTDS Loop Diagnostic_2025"))) %>%
...
## for selecting a subset to run
select_good_rivers <- data.frame("file" = dir(path = file.path(path, "WRTDS Loop Diagnostic_2025"))) %>%
  # Drop the file suffix part of the file name 
  dplyr::mutate(river = gsub(pattern = "\\_Loop\\_Diagnostic.csv", replacement = "", x = file)) %>% 
  separate(river, into=c("LTER","chemical"), sep="__", extra="merge", remove=FALSE) %>% 
  separate(chemical, into=c("Stream_Name","chemical"), sep="_", extra="merge", remove=FALSE) %>% 
  unite(Stream_ID, c("LTER","Stream_Name"), sep="__", remove=FALSE) %>% 
  unite(Stream_element_ID, c("Stream_ID", "chemical"), sep="___", remove=FALSE)

# selecting list of coastal sites
coast_rivers <- select_good_rivers %>% 
  filter(Stream_Name %in% coastal_sites$Stream_Name) %>% 
  pull(Stream_element_ID)

# Identify potential rivers to run through the bootstrap workflow
potential_boots <- data.frame("x" = unique(all_good_rivers)) %>%
  # Identify LTER
...
done_boots <- data.frame("file" = dir(path = file.path(path, "WRTDS Bootstrap Diagnostic Coastal Sites"))) %>%
  # Drop the file suffix part of the file name 
...
bad_boot <- c("Finnish Environmental Institute__KEMIJOKI ISOHAARA 14000___DSi",
              "Finnish Environmental Institute__KEMIJOKI ISOHAARA 14000___NH4",
              "Finnish Environmental Institute__KEMIJOKI ISOHAARA 14000___NOx",
              "Finnish Environmental Institute__KEMIJOKI ISOHAARA 14000___P",
              "Finnish Environmental Institute__Kiiminkij 13010 4-tien s___DSi",
              "Finnish Environmental Institute__Kiiminkij 13010 4-tien s___NH4",
              "Finnish Environmental Institute__Kiiminkij 13010 4-tien s___NOx",
              "Finnish Environmental Institute__Kiiminkij 13010 4-tien s___P")

# Identify rivers to do
#boot_to_do <- setdiff(x = potential_boots, y = c(done_boots, bad_boot_rivers))

boot_to_do <- setdiff(x = coast_rivers, y = c(done_boots,bad_boot))
...
for(river in boot_to_do){
  ...
  write.csv(x = egret_pairs_v2, row.names = F, na = "",
            file.path(path, "WRTDS Bootstrap Outputs Coastal Sites", paste0(out_prefix, "ListPairs_GFN_WRTDS.csv")))
  ...
  write.csv(x = egret_boot_results, row.names = F, na = "",
            file.path(path, "WRTDS Bootstrap Outputs Coastal Sites", paste0(out_prefix, "EGRETCi_GFN_bootstraps.csv")))
  ...
  write.csv(x = egret_boot_summary_v2, file.path(path, "WRTDS Bootstrap Outputs Coastal Sites", paste0(out_prefix, "EGRETCi_GFN_Trend.csv")), row.names = F, na = "")
  ...
  write.csv(x = loop_diagnostic, row.names = F, na = "",
            file.path(path, "WRTDS Bootstrap Diagnostic Coastal Sites", paste0(out_prefix, "Boot_Loop_Diagnostic.csv")))
